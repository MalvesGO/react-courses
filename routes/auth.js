...
module.exports = (server) => {
  ...
  server.post("/auth/login", async (req, res) => {
    ...
    if (email && password) {
          ...
          bcrypt
            ...
            .then(function (response) {
              if (response == true) {
                ...
                return res.json({ message: "Password incorreta!" });
              }
            });
        } else {
      ...
      return res.json({ message: "Insira seu email e password" });
    }
  });
        ...
        bcrypt
          ...
          .then(async function (response) {
            if (response == true) {
              ...
              if (results.length > 0) {
                return res.redirect(
                  `/callback?message=Email verified successfully. You can sign in now.`
                );
              } else {
                return res.redirect(
  ...
  server.post("/auth/delete", async (req, res) => {
    if (req.session && req.session.loggedin) {
      ...
      if (results.length > 0) {
        ...
        res.redirect(
          `/callback?message=There was some problem deleting your account.`
        );
      }
    } else {
  ...
  async function updateUser(body, email) {
    try {
      const results = await pool.query(
        `UPDATE users SET name='${body.name}', address='${body.address}' WHERE email='${email}';`
      );
      return results;
    } catch (e) {
      console.error(e);
